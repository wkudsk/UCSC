...
(define (eval-expression program-line)
  (if (pair? program-line)
    (apply (identifier-ref (car program-line)) 
      (map eval-expression (cdr program-line)))     
...
(define (my-dim expr)
  ...
  (symbol-set! (caar expr)
    (make-vector (round(eval-expression (cadar expr)))))
  ;;insert the symbol into the statement table
  ;;so that the you can look up values in the array
...
(define (my-let expr)
  ...
  (if (pair? (car expr))
    (begin
      (when (not (vector? (symbol-ref (caar expr))))
        (printf "~s not a vector~n" (car expr))
        (usage-exit)
      )
      (vector-set! (symbol-ref (caar expr))
      ;;arrays are supposed to indexed at 0, but scheme indexes at 1
      (- (eval-expression (cadar expr)) 1) 
      (eval-expression (cadr expr)))
    )
    ;;(let i 1)
...
(define (my-goto statement program)
    (when (not (hash-has-key? *label-table* (cadr statement)))
      (printf "~s invald label~n" (cadr statement))
      (usage-exit)
    )
...
;; PRINT statement
(define (my-print str) 
 ;;because of recursion, this being true marks
 ;;end of print statement
 (if (null?  str)
   (newline)
   (begin
      ;;if its a string, just print it, otherwise expressions must be
      ;;evaluated
      (if (string? (car str))  
        (display (car str))
        ;;(display (eval-expression (car str)))      
        (printf " ~s" (eval-expression (car str)))
      )         
      (my-print (cdr str))
    )
  )
...
(for-each
  (lambda (pair) (symbol-set! (car pair) (cadr pair)))
  `(
...
(define (interpret-statement statement program linenumber)
  ;;if there is not statement, then we can continue.
  ;;This in theory should not be checked though.
  (if (null? statement)
    (run-program program (+ linenumber 1))
    (begin
    ;;statement does not exist in the hashtable, then there is an error
    (when (not (hash-has-key? *identifier-table* (car statement)))
      (printf "~s invald statement~n" (car statement))
      (usage-exit)
    )
    (cond
      ;;Since goto and if require different 
      ;;types of expressions and parameters,
      ;;we use a cond to check those
      ...
      (else
        ...
        ;;no jump required with if not a goto or if statement, 
        ;;so just jump to next line.
        (run-program program (+ linenumber 1)))
      )
    )
  )
...
;;runs the meat of the program, 
;;most of the work is done in here
(define (run-program program linenumber)
  ;;program terminates when at the last line, 
  ;;checks to make sure no part of the list is lost.
  ;;since the list works as a linked list, 
  ;;it would be hard to go back when a goto is called
  ;;if only car and cdr were used
    ...
    (let ((line (list-ref program linenumber)))
      (cond
        ...
        ((and (= (length line) 2) (list? (cadr line)))
          ...
          ;;(display (car line))
          ;;(display 'line)
          ;;Have to print the car of the line 
          ;;because its the actual element
          (interpret-statement (car line) program linenumber)
        )     
        ;;empty lines are skipped completely      
        (else
          (run-program program (+ linenumber 1))
      )
      )
...
(define (find-labels program) 
  (when (not (null? program))
    ;;makes sure than there is a number, 
    ;;then checks to see if there is a label and a 
    (when (number? (caar program))
      ;;checks for existing label and operation. 
